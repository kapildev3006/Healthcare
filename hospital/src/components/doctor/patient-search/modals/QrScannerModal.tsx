...
interface QrScannerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onScanSuccess?: (healthId: string, patientName: string) => void;
}
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-lg leading-tight">Scan Health Card QR</h3>
              <p className="text-xs text-blue-100 mt-0.5">
                Point camera at patient&apos;s physical or digital card
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 flex flex-col items-center space-y-5">
          <div className="relative w-64 h-64 bg-slate-950 rounded-2xl overflow-hidden border-2 border-slate-800 flex items-center justify-center shadow-inner">
            ...
            {isScanning && (
              <div className="absolute inset-x-8 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent shadow-lg shadow-cyan-400/50 animate-bounce" />
            )}
            ...
            <div className="text-center space-y-2 pointer-events-none p-4">
              ...
              <p className="text-xs text-slate-400 font-medium">
                Align Health Card QR within frame
              </p>
            </div>
          </div>

          <p className="text-xs text-slate-500 text-center max-w-xs leading-relaxed">
            QR codes contain a non-sensitive server token. Verified doctors receive instantaneous access upon patient identity resolution.
          </p>

          {/* Quick Demo button */}
          <button
            onClick={handleSimulateScan}
            className="w-full py-2.5 px-4 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
          >
